<template>
  <view class="delta-entry" @tap="handleTap">
    <view class="entry-main">
      <view class="entry-icon">打</view>
      <view class="entry-text">
        <view class="entry-title">打手中心</view>
...
<script setup>
  ...
  const entryAction = computed(() => {
    ...
    return deltaStore.statusInfo.entryText;
  });
  const entryDesc = computed(() => {
    ...
    return deltaStore.statusInfo.desc;
  });
  ...
  async function refreshIdentity() {
    ...
    await deltaStore.fetchWorkerIdentity({ showError: false });
  }
